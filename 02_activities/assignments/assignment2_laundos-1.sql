...
SELECT
  customer_id,
  product_id,
  COUNT(*) AS times_purchased_on_distinct_dates
FROM (
  SELECT DISTINCT
    customer_id,
    product_id,
...
WITH sales_by_date AS (
  SELECT
      market_date,
      SUM(COALESCE(quantity, 0) * COALESCE(cost_to_customer_per_qty, 0.0)) AS total_sales
  FROM customer_purchases
...
-- Highest total sales day(s)
SELECT
    'Highest total sales' AS day_type,
    market_date,
    total_sales
FROM ranked
WHERE best_rank = 1
...
-- Lowest total sales day(s)
SELECT
    'Lowest total sales' AS day_type,
    market_date,
    total_sales
FROM ranked
WHERE worst_rank = 1

ORDER BY day_type DESC, market_date; 
...
SELECT
    vp.vendor_name,
    vp.product_name,
    5 * cc.n_customers * vp.original_price AS revenue_per_product
FROM vendor_products vp
CROSS JOIN customer_cnt cc
...
VALUES (27, 'Pumpkin Pie', '12"', 3, 'unit');
...
SET current_quantity = COALESCE((
    ...
    WHERE vi.product_id = pu.product_id
      AND vi.market_date = (
          SELECT MAX(v2.market_date)
          FROM vendor_inventory AS v2
          WHERE v2.product_id = pu.product_id
      )
